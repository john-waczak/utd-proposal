...
- By further partitioning our data into distinct $\mathcal{D}_{\text{train}}, \mathcal{D}_{\text{test}}, \mathcal{D}_{\text{cal}}$ sets we can utilize conformal prediction to generate robust, distribution free, confidence intervals for our ML predictions
- Conformal prediction (CP) is model agnostic
- By using the $\mathcal{D}_{\text{cal}}$ holdout set we can calibrate a heuristic uncertainty score like the residual error $\lvert y - \hat{y}\rvert$ into a suitable confidence interval with coverage $\alpha$.[^2]
